    ...
    where TContainer : IUnifiedContainer<TInterface>, new()
{
    ...
    {
        return _getList().Select(c => c == null ? null : c.Interface).GetEnumerator();
    }
    ...
    {
        _getList().Add(new TContainer { Interface = item });
    }
    ...
    {
        var list = _getList().Select(c => c == null ? null : c.Interface).ToList();
    ...
    {
        _getList().Insert(index, new TContainer { Interface = item });
    }
    ...
    public TInterface this[int index]
    {
        get
        {
            ...
            return container == null ? null : container.Interface;
        }
        set { _getList()[index] = new TContainer { Interface = value }; }
    }
    ...
    private static int IndexOf(IList<TContainer> list, TInterface item)
    {
        return list.FirstIndexWhere(c =>
        {
            if(item == null)
            {
                return c == null || c.Interface == null;
            }
            return c != null && c.Interface == item;
        });
    }
}